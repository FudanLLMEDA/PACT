...
from smoke_checks import check_mcp_tool_payload, require_route_clean
...
class FPGAOptimizerTest(DCPOptimizerBase):
    ...
    async def call_vivado_tool(self, tool_name: str, arguments: dict, timeout: float = 300.0) -> str:
        ...
        try:
            ...
            # Extract text content from result. MCP transport success is not
            # enough: servers can return an application-level error payload.
            if result.content:
                text_parts = [c.text for c in result.content if hasattr(c, 'text')]
                text = "\n".join(text_parts)
                if getattr(result, "isError", False):
                    raise RuntimeError(f"{tool_name} failed: {text}")
                check_mcp_tool_payload(text, f"vivado_{tool_name}")
                return text
            return "(no output)"
    ...
    async def call_rapidwright_tool(self, tool_name: str, arguments: dict, timeout: float = 300.0) -> str:
        ...
        try:
            ...
            # Extract text content from result. Reject JSON {"error": ...}
            # payloads instead of silently continuing with a corrupt design.
            if result.content:
                text_parts = [c.text for c in result.content if hasattr(c, 'text')]
                text = "\n".join(text_parts)
                if getattr(result, "isError", False):
                    raise RuntimeError(f"{tool_name} failed: {text}")
                check_mcp_tool_payload(text, f"rapidwright_{tool_name}")
                return text
            return "(no output)"
    ...
    async def run_test_vexriscv(self, input_dcp: Path, output_dcp: Path) -> bool:
        ...
        try:
            ...
            if opt_result.get("cells_moved", 0) < 1:
                raise RuntimeError(
                    "optimize_cell_placement completed without moving a cell"
                )
            ...
            require_route_clean(route_result)
            error_count = 0
...
async def run_test_mode(input_dcp: Path, output_dcp: Path, debug: bool = False, max_nets: int = 5, run_dir: Optional[Path] = None):
    ...
    if "logicnets" in dcp_name:
        ...
        print(f"[TEST]   - benchmarks/logicnets_jscl_2025.1.dcp")
        print(f"[TEST]   - benchmarks/vexriscv_re-place_2025.1.dcp")
        print(f"[TEST]")
...
Examples:
  ...
  python dcp_optimizer.py benchmarks/logicnets_jscl_2025.1.dcp --test
  python dcp_optimizer.py benchmarks/vexriscv_re-place_2025.1.dcp --test
        """
    )
